...
`V, theta` のループ実行時点での値を使って `fnc_v` を計算する。`fnc_v` の成分として、`fP` や `fQ` の成分がある。`p` の成分の内容(PかQか)に応じて `fP, fQ` の成分を使う。ここでは `p` に含まれない `P, Q` の成分についても計算している。

次に、ヤコビアンの成分を求める。`fnc_v` を v から f(v) へ変換されたものであると考えると、求めるヤコビアンは `∂f/∂v` である。この行列の成分を含む行列として、` dfPi_dthetaj, dfQi_dthetaj, dfPi_dVj, dfQi_dVj` を計算している。例えば１つ目は、`fP` の第 i 成分を `theta` の第 j 成分としたものを (i, j) 成分にもつ行列である。最初にこれらを全要素 0 の nxn 行列として初期化している。ただし、n はノード数である。それぞれの行列を求める際に、`Y` を求めたのと同様にして、対角成分と非対角成分に分けて足し合わせている。ただし、`ViVj` は `V` の縦ベクトルと横ベクトルの積を取ることによって作った、`V` の第 i 成分と第 j 成分を掛け合わせたものを (i, j) 成分に持つ行列である。`sin_tmt` は `theta` の第 j 成分と第 i 成分の差に対して sin を取ったものを (i, j) 成分に持つ行列として作った。`cos_tmt` も同様である。また、`Vi` は `V` の縦ベクトルを横に n 個並べた nxn ベクトルである。`np.where` の * は行列の要素同士の掛け算を意味する。

ヤコビアンを作るには `_v_content, _p_content` を使う。上で求めたヤコビアンの成分と、求めるヤコビアンの各成分を対応づけるために辞書を使う。辞書は `変数[キー]` によって値を取得することができる。ここでは辞書の値としても辞書を返すため、２重の辞書になっている。例えば、`J_dict['P']` とすると `{'θ': dfPi_dthetaj, 'V': dfPi_dVj}` という辞書が返される。その辞書に対してもキーを指定すると、`J_dict['P']['θ']` として、`dfPi_dthetaj` が返される。返される値は numpy 配列であるから、さらに２次元のインデックスを指定して値を得る。どの値を返すかを指定するのが `_p_content` と `_v_content` である。

`fnc_v` に関してもヤコビアンと同様にして辞書を使って作成している。

続く `if` 文は計算に失敗したと判断してからの処理である。

#### ループを抜けた後の処理

計算に成功した場合、ループ内で `V, theta` が決定している。これを `LoadFlow.V` などのような形式にするのがループを抜けた後にする処理の１つである。ここでは numpy 配列から、python の標準の配列に変換したものをプロパティとして設定している。

次に、複素数の計算をするため、`V, theta` の配列を複素数型の numpy 配列に変換している。

`V_dot` は `V, theta` のそれぞれの成分に対して、 Vexp(jθ) を表す複素数の配列である。ここでは lambda 式と `map` 関数を用いている。lambda は無名関数である。通常の関数は `def 関数名` のように定義するが、lambda 式を使うと簡単な処理をその場で定義して使うというようなことができる。ここでは `x` を引数として、`np.exp(1j * x)` を返すような関数を使っている。`map` 関数は `map(処理内容, 配列)` とすることで、配列の各要素に対して処理を施すことができる。ここでは処理内容として lambda 式を使った。`map` の前に * をつけて [] で囲んだのは、配列として展開するという意味である。`map` で `theta` の各要素を処理したのち、それらを配列として展開している。この配列と `V` の掛け算は各要素どうしの掛け算である。

`power` を求める時もヤコビアンの成分を求めた時と同様にして、`V_dot` の縦ベクトルを横に n 個並べたものを使っている。
